...
| 3 | **Recovery Dossier** — one-click police/insurer PDF (timeline + evidence + chain) | 🔴 P0 | ~1 wk | ✅ none (server-side) | ✅ **CLOSED (2026-08-13)** — dashboard "Export Recovery Dossier (PDF)" button (success/error toasts, always enabled); the PDF now includes the owner's **command timeline** (lock/siren/wipe/capture + outcomes) and device alias, and embeds **every** photo (was: first only). RBAC tested: `device_only` shares get 403, `viewer` shares download the PDF. 4 + 2 backend + 5 dashboard tests |
...
| 5 | **Location history + CSV export** — 30-day persistence + export | 🟡 P1 | 1–2 wks | ✅ none | ✅ **CLOSED** — ownership-gated `GET /api/dashboard/locations/{id}/export/csv` (up to 10k rows, decrypted, E.164-safe) + dashboard "Export Location History" button; location persistence is the 90-day retention pipeline |
| 6 | **Magneetar Find Network** (offline BLE mesh) | 🟡 P1 | 2–3 mo | ✅ with BLE scan declaration | 🚧 **Phase 1 LANDED (2026-08-13)** — SOS beacon protocol live: per-request `beacon_token` (request id never on the air), device-facing `GET /api/device/recovery/beacon`, sighting-by-token on the existing Guardian pipeline; Android `SosBeaconBroadcaster` (stolen phone advertises) + `GuardianBeaconScanner` (opt-in guardians auto-report with their own coords, 2h dedup). 12 backend + 16 JVM tests. Remaining: mesh scale-out, beacon-permission UX, battery-aware scheduling |
