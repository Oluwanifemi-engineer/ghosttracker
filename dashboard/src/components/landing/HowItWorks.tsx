...
const STEPS = [
  {
    ...
    description: (
      <>
        ...
        <Link href="/download" className="text-mag-primary hover:text-mag-primary-bright font-semibold underline underline-offset-2 transition-colors">
      ...
      <div className="relative max-w-7xl mx-auto px-5 sm:px-8">
        ...
        <div className="max-w-2xl mx-auto text-center mb-16">
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full border border-white/10 bg-white/[0.03] mb-5">
            <span className="text-[10px] font-mono font-bold tracking-[0.2em] text-white/60">HOW IT WORKS</span>
          </div>
        ...
        <div className="grid md:grid-cols-3 gap-6 relative">
          {/* Connector line */}
          <div className="hidden md:block absolute top-16 left-[16%] right-[16%] h-px bg-gradient-to-r from-mag-primary/40 via-white/15 to-mag-secondary/40" />

          {STEPS.map((step, i) => (
            <div key={step.step} className="relative group">
              <div
                className="relative rounded-2xl border border-white/[0.07] bg-mag-panel/80 backdrop-blur-xl p-7 h-full card-glow"
              >
                {/* Step number */}
                ...
                {/* Icon */}
                <div className="relative w-12 h-12 rounded-xl bg-gradient-to-br from-mag-primary/20 to-mag-secondary/15 border border-white/[0.08] flex items-center justify-center mb-6 group-hover:scale-110 transition-transform duration-300">
