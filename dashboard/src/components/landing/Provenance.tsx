...
export function Provenance() {
  return (
    ...
    >
      ...
      <div className="absolute top-24 right-0 w-[420px] h-[420px] rounded-full bg-mag-primary/6 blur-[120px] pointer-events-none" />
      ...
      <div className="relative max-w-7xl mx-auto px-5 sm:px-8">
        ...
        <div className="max-w-3xl mx-auto text-center mb-14">
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full border border-white/10 bg-white/[0.03] mb-5">
            <span className="text-[10px] font-mono font-bold tracking-[0.2em] text-white/60">OUR STORY</span>
          </div>
        ...
        <div className="grid md:grid-cols-3 gap-5 max-w-5xl mx-auto">
          {ORIGIN_POINTS.map((point) => (
            <div
              key={point.title}
              className="relative group card-glow rounded-2xl border border-white/[0.07] bg-mag-panel/80 backdrop-blur-xl p-7 overflow-hidden"
            >
              ...
              <div className="w-10 h-10 rounded-lg border border-white/[0.08] bg-white/[0.03] flex items-center justify-center mb-5 group-hover:border-mag-primary/30 transition-colors">
                <point.icon size={17} className="text-mag-primary" />
              </div>
              <div className="text-white font-semibold text-sm">{point.title}</div>
              <div className="text-[12.5px] text-white/55 leading-relaxed mt-2">{point.description}</div>
            </div>
          ))}
        </div>

        {/* Honest-signal footnote */}
        <div className="mt-10 flex flex-col sm:flex-row items-center justify-center gap-2 text-[11px] font-mono text-white/35">
